...
function getSidebar2(sidebarPath) {
  ...
  curPath.forEach((item) => {
    if (
      ...
      if (item.indexOf('.md') < 0) {
        curSidebar = {
          title: item,
          path: encodeURI(`/${sidebarPath}/${item}/`),
        };
          ...
          if (!fs.lstatSync(menuPath2).isDirectory()) {
            if (
              ...
              curSidebarChildren.push({
                ...
                path: encodeURI(
                  `/${sidebarPath}/${item}/${childrenItem.replace('.md', '')}`
                ),
              });
            }
          } else {
            let curSidebar = {
              title: childrenItem,
              path: encodeURI(`/${sidebarPath}/${item}/${childrenItem}/`),
            };
            let curMenu = [];
            ...
            childrenPath2.forEach((childrenItem2) => {
              if (
                ...
                curMenu.push({
                  ...
                  path: encodeURI(
                    `/${sidebarPath}/${item}/${childrenItem}/${childrenItem2.replace(
                      '.md',
                      ''
                    )}`
                  ),
        ...
        curSidebar = {
          ...
          path: encodeURI(`/${sidebarPath}/${item.replace('.md', '')}`),
        };
      }
