...
export default {
  reg: '/user/reg',
}> 如果没有token返回false,之后通过token校验用户是否登录。```js
...
> 如果没有 token 返回 false,之后通过 token 校验用户是否登录。
...
> 如果 token 被修改，验证登录失败，清除 token 信息
...
> 将 websocket 对象保存到 vuex 中，方便后续使用
